"""
`FakeBitcoinProxy` allows for unit testing of code that normally uses litecoin
RPC without requiring a running litecoin node.

`FakeBitcoinProxy` has an interface similar to `litecoin.rpc.Proxy`, but does not
connect to a local litecoin RPC node. Hence, `FakeBitcoinProxy` is similar to a
mock for the RPC tests.

`FakeBitcoinProxy` does _not_ implement a full litecoin RPC node. Instead, it
...
from litecoin.core import (
    ...
    # number of satoshis per litecoin
    COIN,
...
from litecoin.wallet import (
    # litecoin address initialized from base58-encoded string
    CBitcoinAddress,
...
def make_address_from_passphrase(passphrase, compressed=True, as_str=True):
    """
    Create a Litecoin address from a passphrase. The passphrase is hashed and
...
class FakeBitcoinProxy(object):
    """
    This is an alternative to using `litecoin.rpc.Proxy` in tests. This class
    ...
    def _call(self, rpc_method_name, *args, **kwargs):
        ...
        python-litecoinlib does not parse.
        """
    ...
    def getblock(self, blockhash, *args, **kwargs):
        ...
        # Note that the actual "getblock" litecoind RPC call from
        # python-litecoinlib returns a CBlock object, not a dictionary.
    ...
    def _batch(self, batch_request_entries):
        ...
        feature found in python-litecoinlib and litecoind RPC.
        """
        ...
        for (idx, request) in enumerate(batch_request_entries):
            ...
            if isinstance(request["params"], list):
                ...
                # matches error message received through python-litecoinrpc
